...
发起行程规划任务（城市攻略层 → 视频行程草案审核 → 混合候选验证 → 逐点调研 → 排线；不做预算估算）。
...
| `preference_mode` | string | 均衡 | 省钱优先 / 体验优先 / 均衡（影响选点倾向，不参与预算计算） |
| `start_date` | string | null | 出发日 YYYY-MM-DD（选填）：提供后 R3 按官方闭馆日真校行程 |

响应：`{"job_id": "..."}`；轮询同 `/api/jobs/{job_id}`（`stage` 依次为：城市攻略层/审核行程草案/圈定景点/调研景点/构建档案/计算路线/生成规划/质量门禁/渲染路书）。
行程任务的 `result` 额外含：`draft_plan`（视频行程草案：逐日逐时段点位 + 审核说明，已作为排线主干应用）、
`pitfall_digest`（避坑专题，每条含评论原文 `quote` 与来源 `source`）、`heat_rank`（热度榜）、
`html_name`（HTML 可视化报告文件名，用下载接口取）。
**不输出预算估算**：报告只给行程规划，预算由用户自行考虑；行程内只保留调研到的确定门票价（标来源）。
