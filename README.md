...
**先看效果（无需安装任何东西）**：[示例路书 · 上海 3 天](examples/示例路书_上海3天.md)（模式 A：城市+天数+住宿） ｜ [示例路书 · 成都 2 天](examples/示例路书_成都2天.md)（模式 B：指定景点清单）——Markdown 在 GitHub 上直接可读；同名 `.html` 为可视化版（下载后双击打开，含行程全景图、详情弹层与来源链接）。
...
HTML 可视化路书（`examples/` 下有两份真实产物：上海 3 天 / 成都 2 天）——逐日时间线、行程全景图、避坑警示卡，每条要点附来源链接（下方动图为**全页滚动**，约 19 秒，取自上海那份）：
...
- **频控保守**：请求间随机间隔 2.5~5 秒，单次运行限量（默认 10 条视频 × 100 条评论），模拟正常浏览节奏；详情页导航另有**会话级预算**（默认 6 次，`.env` 可调）——预算在攻略层/候选验证/逐点调研之间共享，用尽即主动停手，不去撞平台风控。
...
├── examples/            # 免安装预览：两份真实生成产物（Markdown + 可视化 HTML）
│   ├── 示例路书_上海3天.*  # 模式 A：城市 + 天数 + 住宿
│   └── 示例路书_成都2天.*  # 模式 B：指定景点清单
...
| 任务提示"导航预算用尽"或"验证码风控" | 本会话已按安全预算收手 / 被平台风控（详见报告与日志）：**等待 20~30 分钟后重新生成即可补齐**——已采点位命中 7 天缓存，只补缺口。不要连续重试：连续重试会加重风控 |
