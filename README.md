...
**基于抖音真实评论数据的避坑型智能行程规划器**：输入目的地/天数/预算/偏好 → 自动采集抖音相关视频的文案、口播（可转写）与高赞评论 → AI 交叉提炼碎片化攻略信息，对每条信息做**可取/不可取的立场评价** → 生成带**来源引用与置信度分级**的攻略报告（Markdown），以及含**预算明细、避坑专题（附评论原文）、实时热度榜**的逐日行程路书（Markdown + HTML 可视化）。

三大差异化：评论区避坑提取（每条附评论原文引用）、实时热度分析（点赞/评论密度/新鲜度三因子）、全链路来源可溯。产品规格见 `product/PRD.md`。
...
1. **混合候选验证**：大模型圈定 15~20 个候选（景点/美食/体验/购物）→ 逐个抖音验证采集（营销号文案自动过滤）→ 交叉验证筛选出 8~12 个优质候选（也可手动指定清单跳过验证）；
...
6. 输出：逐时段卡片（景点+时长+交通+花费+注意事项）+ **避坑专题（每条附评论原文引用与来源链接）** + **实时热度榜**（点赞/评论密度/近 90 天新鲜度）+ 景点详情卡 + 全部来源；同时产出 Markdown 与 **HTML 可视化**（预算饼图/地图标注/避坑高亮，离线自动降级文本版）。
...
│   ├── heat.py          # 热度指数（点赞/评论密度/新鲜度）+ 避坑专题（纯函数）
│   ├── planner.py       # 行程规划：档案蒸馏 + 预算明细 + 行程生成 + 路书渲染（MD/HTML）
...
│   └── trip.py          # 行程任务编排：候选验证 → 逐点调研 → 档案 → 通行矩阵 → 预算/避坑/热度 → 排线
├── web/
│   └── index.html       # 网页界面
└── data/
