...
6. 输出：逐时段卡片（景点+时长+交通+花费+注意事项+**每日午/晚餐餐厅推荐**）+ **避坑专题（每条附评论原文引用与来源链接）** + **实时热度榜**（点赞/评论密度/近 90 天新鲜度）+ 景点详情卡与**餐厅详情卡（含避坑分析与真实评价摘要）** + 全部来源；同时产出 Markdown 与 **HTML 可视化**（预算饼图/地图标注/避坑高亮，离线自动降级文本版）。
