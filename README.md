...
**独立热度榜**：网页"热度榜"页签输入城市即可查询/刷新——趋势四态判定（本周最火/长盛不衰/正在降温/平稳）基于近 7 天与 60 天以上的内容分布；刷榜只采元数据（点赞与发布时间），不采评论，硬上限 6 景点 × 4 视频。API：`POST /api/heat/refresh` + `GET /api/heat/{city}`。
