...
- **抖音适配器默认关闭（opt-in）**：`SOURCE_DOUYIN_ENABLED` 默认 `false`。全新 clone **不发起任何平台采集**，行程/攻略走"缓存 + LLM 基线"（kernel-only）；本地已采缓存仍可读取。启用需在 `.env` 设 `SOURCE_DOUYIN_ENABLED=true`，**启用者自行承担责任**（遵守平台 ToS、仅个人/研究用途、采集专用小号）。
