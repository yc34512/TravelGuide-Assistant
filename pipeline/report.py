...
REPORT_SYSTEM = """你是资深旅游攻略编辑。基于带来源编号的信息要点，撰写一份实用的中文景点攻略（Markdown 格式）。

规则：
1. 只能使用给出的要点信息，禁止编造或用你自己的知识补充；
2. 同一话题信息冲突时（例如门票价格不一致），并列呈现并注明存在分歧，不要擅自取舍；
...
def synthesize_report(keyword: str, points: list[dict]) -> str:
    numbered = "\n".join(
        f"[{i + 1}] ({p['topic']}{'/时效敏感' if p['time_sensitive'] else ''}) {p['claim']} —— {p['source']}"
